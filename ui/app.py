            ...
            with st.status("🔍 Agent is thinking...", expanded=True) as status:
                try:
                    with logfire.span("📡 Calling RAG Backend"):
                        ...
                        # Guard: check status and content-type before parsing
                        if response.status_code != 200:
                            raise ValueError(f"Backend returned HTTP {response.status_code}: {response.text[:300]}")
                    ...
                    st.error(f"Backend error: {e}")  # now shows the actual reason
                    st.stop()
